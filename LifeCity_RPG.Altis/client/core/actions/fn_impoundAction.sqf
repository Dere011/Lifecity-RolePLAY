...
if((_vehicle getVariable["blackmesa", false])) exitWith {
	hint "Arg, celà ne fonctionne pas. Mais pourquoi ?";
};
if((_vehicle getVariable["isDestroyed", false])) exitWith {
	hint "Le véhicule est détruit, impossible.";
};
...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if(player distance _vehicle > 5) exitWith {hint "Impounding cancelled."; life_action_inUse = false;};
