...
if(!_check) then {
	...
	[_dbName, _uid, "cop_licenses", [(_ret select 5)] call DB_fnc_mresToArray] call iniDB_write;
	[_dbName, _uid, "adminlevel", (_ret select 6)] call iniDB_write;
	[_dbName, _uid, "blacklist", (_ret select 7)] call iniDB_write;
	[_dbName, _uid, "cop_gear", [(_ret select 8)] call DB_fnc_mresToArray]] call iniDB_write;
	[_dbName, _uid, "civ_licenses", [(_ret select 9)] call DB_fnc_mresToArray] call iniDB_write;
	[_dbName, _uid, "arrested", (_ret select 10)] call iniDB_write;
	[_dbName, _uid, "donatorlvl", (_ret select 11)] call iniDB_write;
	[_dbName, _uid, "civ_gear", [(_ret select 12)] call DB_fnc_mresToArray] call iniDB_write;
